...
def selected_boards(
    ...
    """Assign playback slots by configured physical role order."""
    role_to_board = {board.role_id: board for board in boards_by_port.values()}
    selected: list[AutoBoard] = []

    for slot_id, role_id in enumerate(role_order[:max_boards], start=1):
        board = role_to_board.get(role_id)
        if board is None:
            continue
        board.slot_id = slot_id
        selected.append(board)

    for board in boards_by_port.values():
...
def wait_for_start(args: argparse.Namespace) -> tuple[list[AutoBoard], str, bool]:
    ...
    while True:
        ...
        selected = selected_boards(boards_by_port, args.boards, tuple(args.role_order))
        ...
        if now - last_summary >= args.summary_interval:
            ...
            log(f"connected {len(selected)}/{args.boards}; mapped roles={roles}")
            last_summary = now
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Playback slot order by physical role_id, for example: 1 10 19 8",
    )
    args = parser.parse_args()
    if not 1 <= args.boards <= DEFAULT_BOARD_COUNT:
        raise SystemExit("--boards must be 1..4")
    if len(args.role_order) < args.boards:
        raise SystemExit("--role-order must contain at least as many entries as --boards")
    if len(set(args.role_order)) != len(args.role_order):
        raise SystemExit("--role-order must not contain duplicate role IDs")
    return args
...
def main() -> int:
    ...
    log(f"slot role order: {', '.join(str(role_id) for role_id in args.role_order[:args.boards])}")
