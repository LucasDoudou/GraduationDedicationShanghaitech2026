...
class MainWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        QTimer.singleShot(0, self._start_initial_auto_scan)
    ...
    def _start_initial_auto_scan(self) -> None:
        self.log("GUI started; auto scan begins immediately")
    ...
    def _auto_scan_tick(self) -> None:
        ...
        self.log(f"Auto scan tick: {len(self.sessions)}/{MAX_ACTIVE_BOARDS} connected")
        self.auto_connect()
    ...
    def _drain_connect_results(self) -> None:
        ...
        if updated:
            if len(self.sessions) < MAX_ACTIVE_BOARDS:
                self.log(f"Auto scan pending: {len(self.sessions)}/{MAX_ACTIVE_BOARDS} connected, retry in {AUTO_SCAN_INTERVAL_MS // 1000}s")
            else:
                self.log("Auto scan complete: all boards connected")
            self.update_slot_status()
